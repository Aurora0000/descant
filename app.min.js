...
tokenApp.service("tokenService",["$http","$q","$rootScope","descantConfig","localStorageService",function(a,b,c,d,g){this.authenticated=!1;this.token="";this.login=function(e,h){this.authenticated=!1;this.token="";var f=this,k=a.post(d.backend+"/api/auth/login/",{username:e,password:h});k.then(function(a){f.token=a.data.auth_token;f.authenticated=!0;g.set("authToken",f.token);f.setHeader();c.$broadcast("auth:statusChange");return a},function(a){f.token="";f.authenticated=!1;g.remove("authToken");
return b.reject(a)});return k};this.logout=function(){var e=a.post(d.backend+"/api/auth/logout/",{}),h=this;e.then(function(a){h.authenticated=!1;h.token="";g.remove("authToken");h.setHeader();c.$broadcast("auth:statusChange");return a},function(a){h.authenticated=!0;return b.reject(a)});return e};this.getToken=function(){if(this.authenticated)return this.token;var a=g.get("authToken");return null!=a?(this.authenticated=!0,this.token=a):!1};this.setHeader=function(){var b=this.getToken();a.defaults.headers.common.Authorization=
0!=b?"Token "+b:null};this.purgeToken=function(){g.remove("authToken")};this.getAuthStatus=function(){var c=this;if(this.loaded&&!this.error)return b(function(a,b){a(c.user)});var g=a.get(d.backend+"/api/auth/me/");g.then(function(a){c.user=a;c.loaded=!0;c.error=!1;return a},function(a){c.loaded=!0;c.error=!0;return b.reject(a)});return g}}]);var authFormApp=angular.module("descant.directives.authforms",["descant.config","descant.services.tokenservice"]);
...
newTopicApp.directive("newTopicBox",["$location",function(a){return{restrict:"E",templateUrl:"templates/topics/new-topic-box.html",controller:["tokenService","tagService","$rootScope","$http","descantConfig",function(b,c,d,g,e){this.auth=b.authenticated;this.submitting=!1;var h=this;d.$on("auth:statusChange",function(){h.auth=b.authenticated});this.showNTP=!1;this.toggleNTP=function(){this.showNTP=1==this.showNTP?!1:!0};this.addTopic=function(b,c,h){this.submitting=!0;var l;for(l=0;l<h.length;l++)h[l]=
parseInt(h[l].id);var m=this;g.post(e.backend+"/api/v0.1/topics/",{title:b,contents:c,tag_ids:h}).success(function(b){m.submitting=!1;a.path("/topics");m.toggleNTP();d.$broadcast("topics:refresh")}).error(function(a){m.submitting=!1;alert("Error!")})};this.loadTags=function(){return c.getAllTags().$promise}}],controllerAs:"newTopicCtrl"}}]);tagApp=angular.module("descant.directives.taglist",["descant.config","descant.services.tagservice"]);
tagApp.directive("tagList",["$location","tagService",function(a,b){return{restrict:"E",templateUrl:"templates/topics/tag-list.html",controller:["$rootScope","$http","descantConfig",function(a,d,g){this.showTags=!1;this.toggleBox=function(){this.showTags=1==this.showTags?!1:!0};var e=this;this.updateList=function(){e.list=b.getAllTags()};this.updateList()}],controllerAs:"tagCtrl"}}]);var topicListApp=angular.module("descant.directives.topiclist",["descant.config"]);
topicListApp.directive("topicList",["descantConfig",function(a){return{restrict:"E",templateUrl:"templates/topics/topic-list.html",scope:{url:"@"},controller:["$http","$scope","$interval","$rootScope",function(b,c,d,g){var e=this;this.list=[];this.busy=!1;this.offset=0;this.limit=15;this.end=!1;this.updateList=function(){if(!e.busy&&!e.end){e.busy=!0;var d=b.get(a.backend+c.url+"?limit="+e.limit.toString()+"&offset="+e.offset.toString());d.success(function(a){if(0==a.results.length)e.end=!0;else{for(var b=
...
topicViewApp.directive("topicFirstpost",["descantConfig",function(a){return{restrict:"E",templateUrl:"templates/topics/topic-firstpost.html",scope:{postData:"=",topicId:"="},controller:["$http","$scope","$location","$route","tagService","tokenService",function(b,c,d,g,e,h){this.editing=!1;var f=this;f.loaded=!1;var k=b.get(a.backend+"/api/v0.1/topics/"+c.topicId+"/");k.success(function(b){f.post=b;c.postData=b;f.loaded=!0;document.title=f.post.title+" | "+a.forumName;c.user=null!=h.user?h.user.data.id:
-1});k.error(function(a){f.loaded=!0;f.error=!0});this.lock=function(){b.put(a.backend+"/api/v0.1/topics/"+c.topicId+"/",{title:f.post.title,contents:f.post.contents,tag_ids:f.post.tag_ids,is_locked:!0}).success(function(){g.reload()})};this.unlock=function(){b.put(a.backend+"/api/v0.1/topics/"+c.topicId+"/",{title:f.post.title,contents:f.post.contents,tag_ids:f.post.tag_ids,is_locked:!1}).success(function(){g.reload()})};this.edit=function(){this.editing=!this.editing;c.tag_ids_edited=[];c.contents_edited=
f.post.contents;c.title_edited=f.post.title;for(var a=0;a<f.post.tag_ids.length;a++)e.getTagInfo(f.post.tag_ids[a]).then(function(a){null!=a&&c.tag_ids_edited.push(a)})};this.editSubmit=function(){for(var d=c.tag_ids_edited,e=0;e<d.length;e++)d[e]=parseInt(d[e].id);var f=this;b.put(a.backend+"/api/v0.1/topics/"+c.topicId+"/",{title:c.title_edited,contents:c.contents_edited,tag_ids:d}).success(function(a){f.edit()})};this.deleteObj=function(){b["delete"](a.backend+"/api/v0.1/topics/"+c.topicId+"/").success(function(a){d.path("/")})};
...
userListApp.directive("userStats",["descantConfig","tokenService",function(a,b){return{restrict:"E",scope:{userId:"@"},templateUrl:"templates/users/user-stats.html",controller:["$http","$scope",function(c,d){var g=this;b.getAuthStatus().then(function(a){g.user_auth=a.data});var e=c.get(a.backend+"/api/v0.1/users/"+d.userId+"/");e.success(function(a){g.user=a;g.loaded=!0});e.error(function(a){g.loaded=!0;g.error=!0})}],controllerAs:"userCtrl"}}]);
...
controllerApp.controller("ActivateController",["$http","descantConfig","$location","$routeParams",function(a,b,c,d){a=a.post(b.backend+"/api/auth/activate/",{uid:d.uid,token:d.token});a.success(function(a){c.path("/login")});a.error(function(a){alert("Error while activating account!")})}]);
controllerApp.controller("UserCPController",["$http","$location","descantConfig",function(a,b,c){this.options=!1;this.changeUser=function(d,g){a.post(c.backend+"/api/auth/username/",{new_username:d,current_password:g}).success(function(a){b.path("/")}).error(function(a){alert("Error! Is your current password correct?")})};this.changeEmail=function(d){a.patch(c.backend+"/api/auth/me/",{email:d}).success(function(a){b.path("/")}).error(function(a){alert("Error! Is your current password correct?")})};
this.changePassword=function(d,g){a.post(c.backend+"/api/auth/password/",{current_password:g,new_password:d}).success(function(a){b.path("/")}).error(function(a){alert("Error! Is your current password correct?")})};this.showOptions=function(){this.options=!0}}]);var htmlApp=angular.module("descant.filters.html",[]);app.filter("html",["$sce",function(a){return function(b){return a.trustAsHtml(b)}}]);
