...
var localeApp=angular.module("descant.directives.localeselector",["pascalprecht.translate","descant.services.templateservice"]);localeApp.directive("localeSelector",["templateService",function(a){return{restrict:"E",templateUrl:function(){return"templates/"+a.currentTemplateSet()+"/settings/locale-selector.html"},controller:["$translate","descantConfig",function(a,c){this.selectedLanguage="select";this.changeLanguage=function(){a.use(this.selectedLanguage.value)};this.localearr=c.languages}],controllerAs:"localeCtrl"}}]);
var navBarApp=angular.module("descant.directives.navbar",["descant.services.templateservice","descant.config"]);navBarApp.directive("navBar",["templateService",function(a){return{restrict:"E",templateUrl:function(){return"templates/"+a.currentTemplateSet()+"/nav/nav-bar.html"},controller:["descantConfig",function(a){this.name=a.forumName}],controllerAs:"navBarCtrl"}}]);var navApp=angular.module("descant.directives.navbtn",["descant.services.templateservice"]);
...
topicListApp.directive("topicList",["descantConfig","templateService",function(a,b){return{restrict:"E",templateUrl:function(){return"templates/"+b.currentTemplateSet()+"/topics/topic-list.html"},scope:{url:"@"},controller:["$http","$scope","$interval","$rootScope",function(c,b,d,h){var g=this;this.list=[];this.busy=!1;this.offset=0;this.limit=a.apiPaginationLimit;this.end=!1;this.updateList=function(){if(!g.busy&&!g.end){g.busy=!0;var d=c.get(a.backend+b.url+"?limit="+g.limit.toString()+"&offset="+
g.offset.toString());d.success(function(a){if(0==a.results.length)g.end=!0;else{for(var c=a.results,b=0;b<c.length;b++)g.list.push(c[b]);g.offset+=a.results.length;g.busy=!1}});d.error(function(a){g.busy=!1;g.end=!0})}};this.refreshList=function(){g.busy=!0;g.end=!1;var d=c.get(a.backend+b.url+"?limit="+g.offset.toString()+"&offset=0");d.success(function(a){0==a.results.length?g.end=!0:(g.list=a.results,g.busy=!1)});d.error(function(a){g.busy=!1;g.end=!0})};this.stopRefreshList=d(this.refreshList,
45E3);h.$on("topics:refresh",function(){g.refreshList()});h.$on("$destroy",function(){d.cancel(g.stopRefreshList)})}],controllerAs:"topics"}}]);var topicTagsApp=angular.module("angular.directives.topictags",["descant.services.tagservice","descant.services.templateservice"]);
...
topicViewApp.directive("topicFirstpost",["descantConfig","templateService",function(a,b){return{restrict:"E",templateUrl:function(){return"templates/"+b.currentTemplateSet()+"/topics/topic-firstpost.html"},scope:{postData:"=",topicId:"="},controller:["$http","$scope","$location","$route","tagService","tokenService",function(b,e,d,h,g,k){this.editing=!1;var f=this;f.loaded=!1;var l=b.get(a.backend+"/api/v0.1/topics/"+e.topicId+"/");l.success(function(b){f.post=b;e.postData=b;f.loaded=!0;document.title=
f.post.title+" | "+a.forumName;e.user=null!=k.user?k.user.data.id:-1});l.error(function(a){f.loaded=!0;f.error=!0});this.lock=function(){b.put(a.backend+"/api/v0.1/topics/"+e.topicId+"/",{title:f.post.title,contents:f.post.contents,tag_ids:f.post.tag_ids,is_locked:!0}).success(function(){h.reload()})};this.unlock=function(){b.put(a.backend+"/api/v0.1/topics/"+e.topicId+"/",{title:f.post.title,contents:f.post.contents,tag_ids:f.post.tag_ids,is_locked:!1}).success(function(){h.reload()})};this.edit=
function(){this.editing=!this.editing;e.tag_ids_edited=[];e.contents_edited=f.post.contents;e.title_edited=f.post.title;for(var a=0;a<f.post.tag_ids.length;a++)g.getTagInfo(f.post.tag_ids[a]).then(function(a){null!=a&&e.tag_ids_edited.push(a)})};this.editSubmit=function(){for(var d=e.tag_ids_edited,g=0;g<d.length;g++)d[g]=parseInt(d[g].id);var f=this;b.put(a.backend+"/api/v0.1/topics/"+e.topicId+"/",{title:e.title_edited,contents:e.contents_edited,tag_ids:d}).success(function(a){f.edit()})};this.deleteObj=
function(){b["delete"](a.backend+"/api/v0.1/topics/"+e.topicId+"/").success(function(a){d.path("/")})};this.loadTags=function(){return g.getAllTags()}}],controllerAs:"topic"}}]);
