...
tagApp.factory("Tag",["$resource","$cacheFactory","descantConfig",function(a,b,c){return a(c.backend+"/api/v0.1/tags/",{},{query:{method:"GET",cache:!0,isArray:!0}})}]);tagApp.service("tagService",["$http","$q","$rootScope","descantConfig","Tag",function(a,b,c,e,d){this.tags=[];this.getTagInfo=function(a){return d.query().$promise.then(function(b){return b[a-1]})};this.getAllTags=function(){return this.tags=d.query()}}]);
...
tokenApp.service("tokenService",["$http","$q","$rootScope","$route","descantConfig","localStorageService",function(a,b,c,e,d,h){this.authenticated=!1;this.token="";this.login=function(f,e){this.authenticated=!1;this.token="";var g=this,l=a.post(d.backend+"/api/auth/login/",{username:f,password:e});l.then(function(a){g.token=a.data.auth_token;g.authenticated=!0;h.set("authToken",g.token);g.setHeader();c.$broadcast("auth:statusChange");return a},function(a){g.token="";g.authenticated=!1;h.remove("authToken");
...
authApp.directive("authStatus",["$http","tokenService","descantConfig","templateService",function(a,b,c,e){return{restrict:"E",templateUrl:function(){return"templates/"+e.currentTemplateSet()+"/users/auth-status.html"},controller:["$http","$scope","tokenService",function(a,b,c){this.tryAuth=function(){var a=this;c.getAuthStatus().then(function(b){a.user=b.data;a.error=!1},function(b){a.error=!0})};b.tokenServ=c;var e=this;b.$on("auth:statusChange",function(a,b){c.loaded=!1;e.tryAuth()});e.tryAuth()}],
...
newPostApp.directive("newPostBox",["templateService",function(a){return{restrict:"E",templateUrl:function(){return"templates/"+a.currentTemplateSet()+"/posts/new-post-box.html"},scope:{postData:"="},controller:["tokenService","$rootScope","$location","$scope","$http","descantConfig",function(a,c,e,d,h,f){this.submitting=!1;this.auth=a.authenticated;var k=this;c.$on("auth:statusChange",function(){k.auth=a.authenticated});this.showNTP=!1;this.toggleNTP=function(){this.showNTP=1==this.showNTP?!1:!0};
this.addReply=function(a){this.submitting=!0;var b=this;h.post(f.backend+"/api/v0.1/topics/"+d.postData.id+"/replies/",{contents:a}).success(function(a){b.submitting=!1;e.path("/topic/"+d.postData.id);b.toggleNTP();c.$broadcast("topic:refresh")}).error(function(a){b.submitting=!1;alert("Error!")})}}],controllerAs:"newPostCtrl"}}]);var newTopicApp=angular.module("descant.directives.newtopic",["descant.config","descant.services.tokenservice","descant.services.templateservice"]);
newTopicApp.directive("newTopicBox",["templateService",function(a){return{restrict:"E",templateUrl:function(){return"templates/"+a.currentTemplateSet()+"/topics/new-topic-box.html"},controller:["tokenService","tagService","$rootScope","$location","$http","descantConfig",function(a,c,e,d,h,f){this.auth=a.authenticated;this.submitting=!1;var k=this;e.$on("auth:statusChange",function(){k.auth=a.authenticated});this.showNTP=!1;this.toggleNTP=function(){this.showNTP=1==this.showNTP?!1:!0};this.addTopic=
function(a,c,b){this.submitting=!0;var k;for(k=0;k<b.length;k++)b[k]=parseInt(b[k].id);var m=this;h.post(f.backend+"/api/v0.1/topics/",{title:a,contents:c,tag_ids:b}).success(function(a){m.submitting=!1;d.path("/topics");m.toggleNTP();e.$broadcast("topics:refresh")}).error(function(a){m.submitting=!1;alert("Error!")})};this.loadTags=function(){return c.getAllTags().$promise}}],controllerAs:"newTopicCtrl"}}]);var resetCApp=angular.module("descant.directives.resetconf",["descant.config"]);
resetCApp.directive("resetPasswordConfirm",["$location",function(a){return{restrict:"E",templateUrl:"templates/users/password-reset-confirm.html",controller:["$rootScope","$http","descantConfig",function(b,c,e){this.reset=function(b,h,f){c.post(e.backend+"/api/auth/password/reset/confirm/",{uid:b,token:h,new_password:f}).success(function(c){a.path("/")}).error(function(a){alert("Error!")})}}],controllerAs:"resetCtrl"}}]);
...
tagApp.directive("tagList",["tagService","templateService",function(a,b){return{restrict:"E",templateUrl:function(){return"templates/"+b.currentTemplateSet()+"/topics/tag-list.html"},controller:["$rootScope","$http","$location","descantConfig",function(c,b,d,h){this.showTags=!1;this.toggleBox=function(){this.showTags=1==this.showTags?!1:!0};var f=this;this.updateList=function(){f.list=a.getAllTags()};this.updateList()}],controllerAs:"tagCtrl"}}]);
var topicListApp=angular.module("descant.directives.topiclist",["descant.config","descant.services.templateservice"]);
topicListApp.directive("topicList",["descantConfig","templateService",function(a,b){return{restrict:"E",templateUrl:function(){return"templates/"+b.currentTemplateSet()+"/topics/topic-list.html"},scope:{url:"@"},controller:["$http","$scope","$interval","$rootScope",function(c,b,d,h){var f=this;this.list=[];this.busy=!1;this.offset=0;this.limit=a.apiPaginationLimit;this.end=!1;this.updateList=function(){if(!f.busy&&!f.end){f.busy=!0;var d=c.get(a.backend+b.url+"?limit="+f.limit.toString()+"&offset="+
f.offset.toString());d.success(function(a){if(0==a.results.length)f.end=!0;else{for(var c=a.results,b=0;b<c.length;b++)f.list.push(c[b]);f.offset+=a.results.length;f.busy=!1}});d.error(function(a){f.busy=!1;f.end=!0})}};this.refreshList=function(){f.busy=!0;f.end=!1;var d=c.get(a.backend+b.url+"?limit="+f.offset.toString()+"&offset=0");d.success(function(a){0==a.results.length?f.end=!0:(f.list=a.results,f.busy=!1)});d.error(function(a){f.busy=!1;f.end=!0})};this.stopRefreshList=d(this.refreshList,
45E3);h.$on("topics:refresh",function(){f.refreshList()});h.$on("$destroy",function(){d.cancel(f.stopRefreshList)})}],controllerAs:"topics"}}]);var topicTagsApp=angular.module("angular.directives.topictags",["descant.services.tagservice","descant.services.templateservice"]);
...
topicViewApp.directive("topicFirstpost",["descantConfig","templateService",function(a,b){return{restrict:"E",templateUrl:function(){return"templates/"+b.currentTemplateSet()+"/topics/topic-firstpost.html"},scope:{postData:"=",topicId:"="},controller:["$http","$scope","$location","$route","tagService","tokenService",function(b,e,d,h,f,k){this.editing=!1;var g=this;g.loaded=!1;var l=b.get(a.backend+"/api/v0.1/topics/"+e.topicId+"/");l.success(function(b){g.post=b;e.postData=b;g.loaded=!0;document.title=
g.post.title+" | "+a.forumName;e.user=null!=k.user?k.user.data.id:-1});l.error(function(a){g.loaded=!0;g.error=!0});this.lock=function(){b.put(a.backend+"/api/v0.1/topics/"+e.topicId+"/",{title:g.post.title,contents:g.post.contents,tag_ids:g.post.tag_ids,is_locked:!0}).success(function(){h.reload()})};this.unlock=function(){b.put(a.backend+"/api/v0.1/topics/"+e.topicId+"/",{title:g.post.title,contents:g.post.contents,tag_ids:g.post.tag_ids,is_locked:!1}).success(function(){h.reload()})};this.edit=
function(){this.editing=!this.editing;e.tag_ids_edited=[];e.contents_edited=g.post.contents;e.title_edited=g.post.title;for(var a=0;a<g.post.tag_ids.length;a++)f.getTagInfo(g.post.tag_ids[a]).then(function(a){null!=a&&e.tag_ids_edited.push(a)})};this.editSubmit=function(){for(var d=e.tag_ids_edited,f=0;f<d.length;f++)d[f]=parseInt(d[f].id);var g=this;b.put(a.backend+"/api/v0.1/topics/"+e.topicId+"/",{title:e.title_edited,contents:e.contents_edited,tag_ids:d}).success(function(a){g.edit()})};this.deleteObj=
function(){b["delete"](a.backend+"/api/v0.1/topics/"+e.topicId+"/").success(function(a){d.path("/")})};this.loadTags=function(){return f.getAllTags()}}],controllerAs:"topic"}}]);
...
this.deleteObj=function(){var f=a["delete"](d.backend+"/api/v0.1/posts/"+c.post.id+"/");f.success(function(a){e.reload()});f.error(function(a){alert("Unexpected error!")})}}],controllerAs:"postCtrl"}}]);
topicViewApp.directive("postList",["descantConfig","templateService",function(a,b){return{restrict:"E",templateUrl:function(){return"templates/"+b.currentTemplateSet()+"/posts/reply-list.html"},scope:{url:"@"},controller:["$http","$interval","$route","$rootScope","$scope","tokenService",function(b,e,d,h,f,k){var g=this;this.list=[];this.busy=!1;this.offset=0;this.limit=a.apiPaginationLimit;this.end=!1;this.updateList=function(){if(!g.busy&&!g.end){g.busy=!0;var d=b.get(a.backend+f.url+"?limit="+g.limit.toString()+
"&offset="+g.offset.toString());d.success(function(a){if(0==a.results.length)g.end=!0;else{for(var b=a.results,c=0;c<b.length;c++)g.list.push(b[c]);g.offset+=a.results.length;g.busy=!1}});d.error(function(a){g.busy=!1;g.end=!0})}};this.refreshList=function(){g.busy=!0;g.end=!1;var d=b.get(a.backend+f.url+"?limit="+g.offset.toString()+"&offset=0");d.success(function(a){0==a.results.length?g.end=!0:(g.list=a.results,g.busy=!1)});d.error(function(a){g.busy=!1;g.end=!0})};this.stopRefreshList=e(this.refreshList,
45E3);h.$on("topic:refresh",function(){g.refreshList()});this.destroy=function(){e.cancel(g.stopRefreshList)};h.$on("$destroy",function(){g.destroy()});h.$on("$locationChangeSuccess",function(){g.destroy()})}],controllerAs:"posts"}}]);var userListApp=angular.module("descant.directives.userlist",["descant.config","descant.services.templateservice"]);
